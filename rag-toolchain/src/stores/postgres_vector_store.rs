...
mod tests {
    ...
    fn postgres_vector_error_display() {
        ...
        let env_var_error = PostgresVectorStoreError::EnvVarError(var_error);
        assert_eq!(
            env_var_error.to_string(),
            format!("Environment variable error: {}", var_error_message)
        );

        let connection_error = PostgresVectorStoreError::ConnectionError(sqlx::Error::RowNotFound);
        assert_eq!(
        ...
        let table_creation_error =
            PostgresVectorStoreError::TableCreationError(sqlx::Error::RowNotFound);
        assert_eq!(
            table_creation_error.to_string(),
            format!("Table creation error: {}", sqlx_error_message)
        );

        let insert_error = PostgresVectorStoreError::InsertError(sqlx::Error::RowNotFound);
        assert_eq!(
        ...
        let transaction_error =
            PostgresVectorStoreError::TransactionError(sqlx::Error::RowNotFound);
        assert_eq!(
