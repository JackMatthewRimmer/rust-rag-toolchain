use rag_toolchain::{
    ...
    stores::{PostgresVectorStore, DistanceFunction, IndexType },
};
...
async fn main() {
    ...
    let store: PostgresVectorStore =
        PostgresVectorStore::try_new("embeddings", TextEmbeddingAda002, Some(IndexType::HNSW(DistanceFunction::Cosine))
            .await
            .unwrap();
